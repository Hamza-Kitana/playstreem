import { useEffect, useRef } from "react";
import ChatEmoteText from "@/components/ChatEmoteText";
import type { ChatMessage, ChatStatus } from "@/hooks/useKickChat";
import { cn } from "@/lib/utils";
import { useT } from "@/contexts/LocaleContext";

export default function ChatFeed({
  messages: chatMessages,
  status,
  channel,
  className,
}: {
  messages: ChatMessage[];
  status: ChatStatus;
  channel: string | null;
  className?: string;
}) {
  const { messages } = useT();
  const boxRef = useRef<HTMLDivElement | null>(null);
  const labels: Record<ChatStatus, string> = {
    idle: messages.nav.offline,
    connecting: messages.common.loading,
    live: messages.nav.live,
    error: messages.pages.chat.statusError,
  };

  useEffect(() => {
    const el = boxRef.current;
    if (el) el.scrollTop = el.scrollHeight;
  }, [chatMessages]);

  const live = status === "live";

  return (
    <div className={cn("glass flex flex-col overflow-hidden rounded-3xl", className)}>
      <div className="flex items-center justify-between border-b border-border/60 px-4 py-3">
        <div className="flex items-center gap-2">
          <span
            className={cn(
              "size-2.5 rounded-full",
              live ? "bg-primary animate-pulse-glow" : "bg-muted-foreground",
            )}
          />
          <span className="text-sm font-bold">{labels[status]}</span>
        </div>
        <span className="max-w-[9rem] truncate text-xs text-muted-foreground">
          {channel ?? messages.pages.chat.noChannel}
        </span>
      </div>

      <div ref={boxRef} className="min-h-72 flex-1 space-y-2 overflow-y-auto p-3">
        {chatMessages.length === 0 ? (
          <p className="px-2 py-10 text-center text-sm text-muted-foreground">
            {messages.pages.chat.waiting}
          </p>
        ) : (
          chatMessages.slice(-100).map((m) => (
            <div key={m.key} className="animate-chat-in rounded-xl bg-secondary/40 px-3 py-2">
              <span className="text-sm font-bold" style={{ color: m.color }}>
                {m.user}
              </span>
              <span className="mx-1 text-muted-foreground">:</span>
              <ChatEmoteText text={m.text} className="text-sm leading-7" size="md" />
            </div>
          ))
        )}
      </div>
    </div>
  );
}
